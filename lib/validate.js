...
const isValidPublicDomain = (hostname) => {
  if(!isValidDomain(hostname)) return false;

  const tld = hostname.split('.').pop(); // "com" from

  return tld && tlds.includes(tld);
};

const isValidPublicIp = (hostname) => ipaddr.isValid(hostname) && isPrivateIp(hostname);

const isValidUrl = url => (url.protocol != null) &&
  url.protocol.match(/^http[s]?:/) &&
  url.slashes &&
  (url.hostname != null) &&
  (isValidPublicDomain(url.hostname) ||
    isValidPublicIp(url.hostname));
