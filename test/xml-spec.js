...
`
  ...
  it('should set attributes on arrays the new way', function() {
    const vars = {
      xml_path: {
        'foo.bar.0.@id': '123',
        'foo.bar.0.#text': 'baz'
...
`
  ...
  it('should set attributes on arrays the old way', function() {
    const vars = {
      xml_path: {
        'foo.bar.0@id': '123',
        'foo.bar.0': 'baz'
...
describe('XML validation', function() {
  ...
  it('should require valid method', () => assert.equal(integration.validate({url, method: 'HEAD'}), 'Unsupported HTTP method - use POST, PUT, PATCH'));
