...
const request = function(vars) {
  ...
  // check and map any nested_value arguments
  if (vars.nested_value) {
    const nested = Object.keys(vars.nested_value);
    for (elem of nested) {
      _.set(json, elem, vars.nested_value[elem]);
    }
  }
...
request.variables = () => [
  ...
  { name: 'nested_value.*', description: 'Takes a value and maps it to the body in the specified dot notated path', type: 'wildcard', required: false },
