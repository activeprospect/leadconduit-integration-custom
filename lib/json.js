...
const request = function(vars) {
  ...
  const preferences = Object.keys(vars).filter(varName => varName.includes('preferences'));
  if (preferences.length) {
    for (preference of preferences) {
      _.set(json, preference, vars[preference]);
    }
  }
...
request.variables = () => [
  ...
  { name: 'preferences.*', description: 'An', type: 'string'},
