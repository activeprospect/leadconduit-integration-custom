...
describe('XML validation', function() {
  ...
  it('should allow valid content-type header', () => assert.isUndefined(integration.validate({url: 'http://foo', header: { 'Content-Type': 'application/xml' }})));

  it('should not allow invalid content-type header', () => assert.equal(integration.validate({url: 'http://foo', header: { 'Content-Type': 'text/plain' }}), 'Invalid Content-Type header value'));
