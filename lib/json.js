...
const request = function(vars) {
  ...
  if (vars.json_parameter) {
    const bodyOrUndefined = (! _.isEmpty(JSON.parse(body)) ) ? body : undefined;
    
    body = helpers.parameterize(vars.json_parameter, bodyOrUndefined, vars.extra_parameter, vars.nested_extra_parameter);
...
request.variables = () => [
  ...
  { name: 'nested_extra_parameter.*', description: 'Extra JSON object to include in the URL', type: 'wildcard', required: false },
